 ...
 * to the buffer in the device_ctx
 */
 ...
 * ctx: Ptr to device_ctx, which contains various processing parameters as detailed above.
 ...
 * device_http_postfile performs a post operation which uploads a file, this is specified by filename.
 *
 * Return value is the HTTP status value from the server (e.g. 200 for HTTP OK)
 */

long device_http_get
  (iot_logger_t *lc, device_ctx *ctx, const char *url, void *writefunc, devsdk_error *err);

long device_http_delete
  (iot_logger_t *lc, device_ctx *ctx, const char *url, void *writefunc, devsdk_error *err);
...
long device_http_postfile
  (iot_logger_t *lc, device_ctx *ctx, const char *url, const char *filename, void *writefunc, devsdk_error *err);
